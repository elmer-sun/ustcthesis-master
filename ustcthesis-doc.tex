...
启用盲审模式。按照研究生学位论文评阅系统的盲审论文参考模板\footnote{
  \url{https://pyxt.ustc.edu.cn/images/doc_demo.pdf}}，
  隐去中英文封面的作者姓名、导师姓名，删除致谢部分内容，
  研究成果使用 \env{achievements*} 环境的内容。
  可选：\opt{false}（默认），\opt{true}。
\begin{latex}
  \documentclass[review=true]{ustcthesis}
\end{latex}
...
声明页由 \cs{statement} 命令生成。模板会根据 \opt{secret-level} 和
...
\begin{itemize}
  ...
  \item 科研成果（用于盲审）：\env{achievements*}
